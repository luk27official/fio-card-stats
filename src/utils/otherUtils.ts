...
export const createTransactionNameMapping = <T>(
    data: T[],
...
export const getUniqueItems = <T>(
    ...
    return data.reduce((acc: string[], item) => {
        ...
        if (hideDuplicates) {
            ...
            const isDuplicate = acc.some(existingItem =>
                normalizeTransactionName(existingItem) === normalizedIt
            );
...
export const calculateTotalSum = (data: Array<{ Objem?: string; Měna?: string; }>, targetCurrency: Currency = "CZK"): string => {
    let sum = 0;
